...
// A probe migration one past the latest registered version (v6 = m0006 session_lifecycle), so it
// never collides with a shipped migration as the real history grows.
const dummyV7: Migration = {
  version: 7,
...
describe("schema evolution: idempotency, version bump, refuse-newer (FR-007/FR-008; SC-004/SC-005)", () => {
  it("is idempotent — a second migrate() with nothing pending applies 0", async () => {
    ...
    expect(first.applied).toBe(6); // m0001 + m0002 + m0003 + m0004 + m0005 + m0006

    const second = await migrate(db);
    expect(second).toEqual({ from: 6, to: 6, applied: 0 });
  });

  it("applies only the newly-registered migration on a version bump (6 → 7)", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db); // at v6 (latest registered)

    const result = await migrate(db, [...registered, dummyV7]);
    expect(result).toEqual({ from: 6, to: 7, applied: 1 });

    const uv = await db.select<{ user_version: number }>("PRAGMA user_version");
    expect(uv[0].user_version).toBe(7);
  ...
  it("refuses to operate on a store newer than the app knows about", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db, [...registered, dummyV7]); // store advanced to v7

    // An older build that only knows the registered set (latest v6) must refuse, not risk corruption.
