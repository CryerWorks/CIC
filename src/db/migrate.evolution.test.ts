...
// A probe migration one past the latest registered version (v3 = m0003 vaults), so it never
// collides with a shipped migration as the real history grows.
const dummyV4: Migration = {
  version: 4,
...
describe("schema evolution: idempotency, version bump, refuse-newer (FR-007/FR-008; SC-004/SC-005)", () => {
  it("is idempotent — a second migrate() with nothing pending applies 0", async () => {
    ...
    expect(first.applied).toBe(3); // m0001 + m0002 + m0003

    const second = await migrate(db);
    expect(second).toEqual({ from: 3, to: 3, applied: 0 });
  });

  it("applies only the newly-registered migration on a version bump (3 → 4)", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db); // at v3 (latest registered)

    const result = await migrate(db, [...registered, dummyV4]);
  ...
  it("refuses to operate on a store newer than the app knows about", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db, [...registered, dummyV4]); // store advanced to v4
