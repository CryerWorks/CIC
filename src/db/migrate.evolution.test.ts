...
// A probe migration one past the latest registered version (v8 = m0008 project_authoring), so it
// never collides with a shipped migration as the real history grows.
const dummyV9: Migration = {
  version: 9,
...
describe("schema evolution: idempotency, version bump, refuse-newer (FR-007/FR-008; SC-004/SC-005)", () => {
  it("is idempotent — a second migrate() with nothing pending applies 0", async () => {
    ...
    expect(first.applied).toBe(8); // m0001 + m0002 + m0003 + m0004 + m0005 + m0006 + m0007 + m0008

    const second = await migrate(db);
    expect(second).toEqual({ from: 8, to: 8, applied: 0 });
  });

  it("applies only the newly-registered migration on a version bump (8 → 9)", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db); // at v8 (latest registered)

    const result = await migrate(db, [...registered, dummyV9]);
    expect(result).toEqual({ from: 8, to: 9, applied: 1 });

    const uv = await db.select<{ user_version: number }>("PRAGMA user_version");
    expect(uv[0].user_version).toBe(9);
  ...
  it("refuses to operate on a store newer than the app knows about", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db, [...registered, dummyV9]); // store advanced to v9

    // An older build that only knows the registered set (latest v8) must refuse, not risk corruption.
