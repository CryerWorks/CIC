...
// A probe migration one past the latest registered version (v10 = m0010 feynman_gaps), so it
// never collides with a shipped migration as the real history grows.
const dummyV11: Migration = {
  version: 11,
...
describe("schema evolution: idempotency, version bump, refuse-newer (FR-007/FR-008; SC-004/SC-005)", () => {
  it("is idempotent — a second migrate() with nothing pending applies 0", async () => {
    ...
    expect(first.applied).toBe(10); // m0001–m0010

    const second = await migrate(db);
    expect(second).toEqual({ from: 10, to: 10, applied: 0 });
  });

  it("applies only the newly-registered migration on a version bump (10 → 11)", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db); // at v10 (latest registered)

    const result = await migrate(db, [...registered, dummyV11]);
    expect(result).toEqual({ from: 10, to: 11, applied: 1 });

    const uv = await db.select<{ user_version: number }>("PRAGMA user_version");
    expect(uv[0].user_version).toBe(11);
  ...
  it("refuses to operate on a store newer than the app knows about", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db, [...registered, dummyV11]); // store advanced to v11

    // An older build that only knows the registered set (latest v10) must refuse, not risk corruption.
