...
// A probe migration one past the latest registered version (v4 = m0004 srs_scoping), so it never
// collides with a shipped migration as the real history grows.
const dummyV5: Migration = {
  version: 5,
...
describe("schema evolution: idempotency, version bump, refuse-newer (FR-007/FR-008; SC-004/SC-005)", () => {
  it("is idempotent — a second migrate() with nothing pending applies 0", async () => {
    ...
    expect(first.applied).toBe(4); // m0001 + m0002 + m0003 + m0004

    const second = await migrate(db);
    expect(second).toEqual({ from: 4, to: 4, applied: 0 });
  });

  it("applies only the newly-registered migration on a version bump (4 → 5)", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db); // at v4 (latest registered)

    const result = await migrate(db, [...registered, dummyV5]);
  ...
  it("refuses to operate on a store newer than the app knows about", async () => {
    const db = NodeSqlExecutor.open();
    await migrate(db, [...registered, dummyV5]); // store advanced to v5

    // An older build that only knows the registered set (latest v3) must refuse, not risk corruption.
