...
<!-- SPECKIT START -->
**Active feature:** `007-course-moc` — **planned** (spec + plan complete; not yet implemented): Course authoring (F1) that materializes each Course as an Obsidian **MOC** Markdown file, plus read-back on app open / manual rescan (F7). Three layers: (1) a **pure MOC document module** `src/features/courses/moc/` — render / merge / parse the locked v0.7 body, the sole home of the `<!-- cic:* -->` marker contract (R2; capability is wrapped in `cic:capability` markers so *all* app-managed sections are uniformly marker-delimited — a flagged PRD-template clarification); milestone lines carry a hidden `<!-- cic:m id=… status=… -->` comment for faithful 3-state + stable-identity round-trip (R3). (2) a **sync layer** `src/features/courses/sync/` — `materializeCourse` (render/merge → `VaultWriter`, marker-scoped so user regions like `## Reflections` are never clobbered; drift → surface + "Reload & reapply" via `overwrite`) and `rescanCourses` (list → discriminate by `cic-type: course` frontmatter → parse → upsert-by-`cic-id` + delete-missing milestones; imports unknown CIC MOCs, auto-creates named Domain/Campaign). (3) the **Courses screen** `src/features/courses/` (hook mirroring `useDomains`, form, milestones editor) gating on `useVaultState()`. No new SQLite schema — additive repo fns only (`courses`/`milestones` + new `campaigns.ts`). Pure module + sync are Vitest-tested (node temp vault + `node:sqlite`); the live `VaultWriter`/Obsidian round-trip is the user's `tauri dev` quickstart check. See [specs/007-course-moc/plan.md](specs/007-course-moc/plan.md), `research.md`, `data-model.md`, `contracts/`, `quickstart.md`.

**Prior feature:** `006-vault-config` — **implemented**: the user-facing gate that points CIC at an Obsidian vault (OS chooser via `tauri-plugin-dialog`, runtime scope grant via the custom `grant_vault_access` Rust command, path persisted in the SQLite `settings` table behind `getSetting`/`setSetting`, read-only note-count reachability check). `VaultProvider` mirrors `DbProvider` (mounted above the store-gate, reads `useDbState()`) and holds the active-vault handle (`useVault()` → the 005 `{ reader, writer }`) with a `checking · unset · ready · unavailable` machine; `/vault` renders onboarding/status/recovery, Dashboard shows a first-run banner. Tauri pieces sit behind `FolderPicker`/`VaultConnector` seams (`src/app/providers/vault/`), unit-tested Tauri-free. See [specs/006-vault-config/plan.md](specs/006-vault-config/plan.md) & artifacts. (`001`–`006` complete.)
<!-- SPECKIT END -->
