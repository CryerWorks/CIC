...
<!-- SPECKIT START -->
**Active feature:** `006-vault-config` — **implemented**: the user-facing gate that points CIC at an Obsidian vault. Choose the folder (OS chooser via `tauri-plugin-dialog`), authorize **only** that folder at runtime (the custom `grant_vault_access` Rust command — `app.fs_scope().allow_directory`, flagged), persist the path as app state in the new SQLite `settings` key-value table (migration `m0002`, behind `getSetting`/`setSetting`), and confirm reachability with a **read-only** Markdown-note count (no probe writes). `VaultProvider` (mirrors `DbProvider`, mounted above the store-gate so it reads `useDbState()`) holds the single active-vault handle (`useVault()` → the 005 `{ reader, writer }`) with a `checking · unset · ready · unavailable` state machine + `chooseVault`/`changeVault`/`retry`; the `/vault` screen renders onboarding / status / recovery, and the Dashboard shows a first-run banner when unset. Deep Tauri pieces (`@tauri-apps/plugin-dialog`, the scope-grant `invoke` + `createVault`) sit behind `FolderPicker`/`VaultConnector` seams (`src/app/providers/vault/`), so the provider is unit-tested Tauri-free (real `node:sqlite` + fakes). Feature 005 unchanged. The Tauri folder picker + scope grant are the only non-unit-tested surfaces (the user's `tauri dev` runtime check). See [specs/006-vault-config/plan.md](specs/006-vault-config/plan.md), `research.md`, `data-model.md`, `contracts/`, `quickstart.md`. (`001`–`006` complete.)

**Prior feature:** `005-vault-layer` — **implemented**: the `src/vault/` spine has landed (Constitution I/IV). A `VaultFs` seam (`fs.ts`) with `adapters/tauri.ts` (sole `@tauri-apps/plugin-fs` importer) + `adapters/node.ts` (tests); pure `paths.ts` boundary; `fingerprint.ts` (mtime + SHA-256, content identity by hash); `VaultReader`/`VaultWriter` (atomic temp→rename writes, never-clobber conflict detection via the 003 `vault_writes` repo behind a `VaultWriteLog` seam); gray-matter frontmatter validated by caller zod schemas; `createVault` composition root + public barrel `index.ts`. No UI; the full vault suite (30 tests) + the `vault_writes` repo test run green against `node:fs`/`node:sqlite` temp dirs. The Tauri adapter + GUI runtime check (quickstart) are the user's `tauri dev` surface. See [specs/005-vault-layer/plan.md](specs/005-vault-layer/plan.md), `research.md`, `data-model.md`, `contracts/`, `quickstart.md`. (`001`–`005` complete.)
<!-- SPECKIT END -->
